...
export default function ExtractModal({ isOpen, onClose }) {
  ...
  const handleExtract = async () => {
    ...
    try {
      const extractedPdfBytes = await extractPages(files[0], pageNumbers, (progressValue, text) => {
      ...
      toast.success(`Pages extracted successfully! ${pageNumbers.length} page(s) extracted`);
      onClose();
    } catch (error) {
      console.error('Error extracting pages:', error);
      toast.error('Error extracting pages. Please try again.');
    } finally {
