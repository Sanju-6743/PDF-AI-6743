...
/**
 * Extract text using Optic API (primary). Falls back to Gemini on failure.
 * @param {Buffer} pdfBuffer - PDF file buffer
 * @returns {Promise<string>} - Extracted text
 */
export async function extractTextWithOptic(pdfBuffer) {
  const opticKey = process.env.OPTIC_API_KEY;
  if (!opticKey) {
    console.warn('OPTIC_API_KEY not set. Falling back to Gemini.');
    return extractTextWithGemini(pdfBuffer);
  }

  try {
    // NOTE: Replace URL/path if your Optic endpoint differs
    const response = await axios.post(
      'https://api.optic.ai/v1/ocr/extract',
      pdfBuffer,
      {
        headers: {
          'Authorization': `Bearer ${opticKey}`,
    ...
    // Assuming response contains { text: string }
    if (response?.data?.text) return response.data.text;

    // Fallback if schema differs
    if (typeof response.data === 'string') return response.data;

    console.warn('Optic response missing text. Falling back to Gemini.');
    return extractTextWithGemini(pdfBuffer);
  } catch (err) {
    console.error('Optic OCR failed. Falling back to Gemini.', err?.response?.data || err.message);
    return extractTextWithGemini(pdfBuffer);
  }
}
